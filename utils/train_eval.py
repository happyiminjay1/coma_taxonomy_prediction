...
FEAT_AVAIL = True
ReconLoss = 'encoder' # L2, CE, encoder
...
def run(model, train_loader, test_loader, epochs, optimizer, scheduler, writer, exp_name, device, args):
    ...
    for epoch in range(1, epochs + 1):
        ...
        tax_acc = test(model, test_loader, epoch, loss_weight, 0, 30, device)
...
def loss_jocor(y_1, y_2, t, forget_rate, ind, co_lambda=0.1):
    ...
    kl_left = kl_loss_compute(y_1, y_2,reduce=False)
    kl_right = kl_loss_compute(y_2, y_1, reduce=False)
...
def train(model, optimizer, train_loader, loss_weight, start_idx, end_idx, device):
    ...
    if ReconLoss == 'encoder' :
        model.de_layers.requires_grad = False
    ...
    for i in tqdm.tqdm(range(start_idx,end_idx)) :
        ...
        x_contact = sample['contact'].to(device)

        ######## FEATUR CONCATING ##########

        x = torch.cat((x,x_contact),dim=2)      

        out, pred_taxonomy = model(x)

        ######## Recon Loss + Contact Recon Loss ##########
        ...
        if ReconLoss == 'encoder' :
            ...
            recon_loss = loss_l2(out[:,:,:4], x[:,:,:4])

            taxonomy_loss = criterion_taxonomy(pred_taxonomy,gt_taxonomy.to(device))

            loss = recon_loss + taxonomy_loss
            ...
            recon_loss = F.l1_loss(out[:,:,:3], x[:,:,:3], reduction='mean')

            contact_hand = out[:,:,3:13]

            gt_contact_map = val_to_class(x_contact).squeeze(2).long().to(device)

            contact_classify_loss = criterion(contact_hand.permute(0, 2, 1), gt_contact_map)

            taxonomy_loss = criterion_taxonomy(pred_taxonomy,gt_taxonomy.to(device))

            loss = recon_loss + contact_classify_loss + taxonomy_loss

            total_recon_loss += recon_loss.item()

            total_contact_loss += contact_classify_loss.item()
        
        loss.backward()

        total_taxonomy_loss += taxonomy_loss.item()
        
        optimizer.step()
        
    return total_recon_loss / count_taken , total_contact_loss / count_taken , total_taxonomy_loss / count_taken
...
def test(model, test_loader, epoch, loss_weight,start_idx, end_idx, device) :
    ...
    return  total_taxonomy_acr / count_taken
