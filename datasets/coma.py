...
def apply_rotation(vertices, rotation_matrix):
        """
        Apply a 4x4 rotation matrix to a set of vertices.

        :param vertices: NumPy array of shape (N, 4), where N is the number of vertices.
        :param rotation_matrix: NumPy array of shape (4, 4) representing the rotation matrix.
        :return: NumPy array of transformed vertices.
        """

        homogenized_vertices = np.hstack([vertices, np.ones((vertices.shape[0], 1))])

        # Check if the rotation matrix is 4x4
        if rotation_matrix.shape != (4, 4):
            raise ValueError("Rotation matrix must be 4x4")

        # Transform the vertices
        transformed_vertices = homogenized_vertices @ rotation_matrix.T  # Matrix multiplication
...
class CoMA(InMemoryDataset):
    ...
    def raw_file_names(self):
        return '/scratch/minjay/coma_refiner/template/hand_mesh_template.obj'
    ...
    def process(self):
        ...
        fps_train = glob('/home/minjay/Artiboost_PreProcess/Sample/MANO_GT_train/*.ply')
        fps_test = glob('/home/minjay/Artiboost_PreProcess/Sample/MANO_GT_test/*.ply')
        ...
        for i in range(len(fps_train)) :
            sort_fps_GT_train.append(f'/home/minjay/Artiboost_PreProcess/Sample/MANO_GT_train/{i}.ply')
            sort_fps_PRED_train.append(f'/home/minjay/Artiboost_PreProcess/Sample/MANO_PRED_train/{i}.ply')

        for i in range(len(fps_test)) :
            sort_fps_GT_test.append(f'/home/minjay/Artiboost_PreProcess/Sample/MANO_GT_test/{i}.ply')
            sort_fps_PRED_test.append(f'/home/minjay/Artiboost_PreProcess/Sample/MANO_PRED_test/{i}.ply')

        train_data_list, test_data_list = [], []
        train_data_list_total, test_data_list_total = [], []

        for idx, fp in enumerate(tqdm(sort_fps_GT_train)):

            data_gt = read_mesh(sort_fps_GT_train[idx]) 
            data_pred = read_mesh(sort_fps_PRED_train[idx])
            data_pred.x = data_pred.x * 1000


            hoi_seq = idx

            with open(f'/home/minjay/Artiboost_PreProcess/Sample/contactopt_preprocess_train/{hoi_seq}.pkl', 'rb') as file:
                data_feature_output = pickle.load(file)

            with open(f'/home/minjay/Artiboost_PreProcess/Sample/hand_object_mesh_joint_train/{hoi_seq}.pkl', 'rb') as file:
                data_info = pickle.load(file)

            hoi_feature = data_feature_output['hand_feats_pred_aug']
            contactmap = data_feature_output['hand_gt_contact']
            contactmap_pred = data_feature_output['hand_pred_contact']
            taxonomy = data_info['taxonomy']
            joints_gt = data_info['joints_pred']  # 순서를 바꿔서 데이터를 줌 .. create_Dataset_decoder_Dataset.py에서 251 line에서 실수함.
            joints_pred = data_info['joints_gt'] * 1000 # 마찬가지

            # print(joints_gt) # 1
            # print(joints_pred) # 10^-3

            # print(data_gt.x) # 1
            # print(data_pred.x) # 10^-3
            ...
            # 1. #
            ######################################
            ## check whether it is well applied ##
            ######################################
            #print(data_gt.x.shape)

            is_right_vec = torch.ones(joints_gt.shape[0]) * True

            normalized_kps, normalization_mat = transform_to_canonical(joints_gt, is_right=is_right_vec)

            transformed_vertices = apply_rotation(data_gt.x.numpy()*1000,normalization_mat[0].numpy())

            data_gt.x = torch.tensor(transformed_vertices,dtype=torch.float32) - normalized_kps[0][0]
            
            #data_gt.x = torch.tensor([data_gt.x], dtype = torch.float32)

            joints_gt = normalized_kps

            is_right_vec = torch.ones(joints_pred.shape[0]) * True
            normalized_kps, normalization_mat = transform_to_canonical(joints_pred, is_right=is_right_vec)
        
            transformed_vertices = apply_rotation(data_pred.x.numpy()*1000,normalization_mat[0].numpy())

            data_pred.x = torch.tensor(transformed_vertices,dtype=torch.float32) - normalized_kps[0][0]

            #data_pred.x = torch.tensor([data_gt.x], dtype = torch.float32)

            joints_pred = normalized_kps

            # MANO_right= ManoLayer(
            #     mano_root='data/mano', side='right', use_pca=True, ncomps=45, flat_hand_mean=False)

            # display_hand({
            #     'verts': data_pred.x.unsqueeze(0),
            #     'joints': joints_pred
            # },
            # mano_faces=MANO_right.th_faces.cpu().data.numpy())

            # display_hand({
            #     'verts': data_gt.x.unsqueeze(0),
            #     'joints': joints_gt
            # },
            # mano_faces=MANO_right.th_faces.cpu().data.numpy())
            ...
            ####
            # trans_to_zero = data_gt.x[0]

            # data_gt.x   =  data_gt.x   - trans_to_zero

            # data_pred.x =  data_pred.x - data_pred.x[0]
            # joints_gt   =  joints_gt / 1000  - trans_to_zero
 
            # joints_gt = joints_gt * 1000
            ####

            # display_hand({
            #     'verts': data_gt.x.unsqueeze(0) * 1000,
            #     'joints': joints_gt * 1000
            # },
            # mano_faces=MANO_right.th_faces.cpu().data.numpy())

            # 2. #
            ## 'trans_gt':data_gt.x[0], 'trans_pred':data_pred.x[0] ##

            data_total = {'hand_mesh_gt':data_gt, 'hand_mesh_pred':data_pred, 'hoi_feature':hoi_feature,'contactmap':contactmap,'taxonomy':taxonomy,'trans_gt':data_gt.x[0], 'trans_pred':data_pred.x[0], 'contactmap_pred':contactmap_pred, 'hoi_seq_id':hoi_seq, 'joints_gt':joints_gt}

            if self.split == 'interpolation':

                train_data_list_total.append(data_total)

                train_data_list.append(data_pred)
        ...
        for idx, fp in enumerate(tqdm(sort_fps_GT_test)):

            data_gt = read_mesh(sort_fps_GT_test[idx])
            data_pred = read_mesh(sort_fps_PRED_test[idx])
            data_pred.x = data_pred.x * 1000

            hoi_seq = idx

            with open(f'/home/minjay/Artiboost_PreProcess/Sample/contactopt_preprocess_test/{hoi_seq}.pkl', 'rb') as file:
                data_feature_output = pickle.load(file)

            with open(f'/home/minjay/Artiboost_PreProcess/Sample/hand_object_mesh_joint_test/{hoi_seq}.pkl', 'rb') as file:
                data_info = pickle.load(file)
            ...
            taxonomy = data_info['taxonomy']
            joints_gt = data_info['joints_pred'] # 순서를 바꿔서 데이터를 줌 .. create_Dataset_decoder_Dataset.py에서 251 line에서 실수함.
            joints_pred = data_info['joints_gt'] * 1000
            ...
            # print(normalized_kps)
            # print(normalization_mat.shape)
            transformed_vertices = apply_rotation(data_gt.x.numpy()*1000,normalization_mat[0].numpy())
            ...
            if self.split == 'interpolation':
                ...
                test_data_list.append(data_pred)
        ...
        with open('/scratch/minjay/coma_refiner/data/CoMA/processed/interpolation/training_total.pkl', 'wb') as file:
            pickle.dump(train_data_list_total, file)
        with open('/scratch/minjay/coma_refiner/data/CoMA/processed/interpolation/test_total.pkl', 'wb') as file:
